...
const roleLabel: Record<string, string> = {
  admin: "Administrator", teacher: "Teacher", parent: "Guardian", student: "Student",
};

function NavContent({ role, userName, userEmail, onNavClick }: SidebarProps & { onNavClick?: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full sidebar-root">
      {/* Logo */}
      <div className="px-4 py-4 flex-shrink-0" style={{ borderBottom: "1px solid hsl(var(--sidebar-border))" }}>
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            <p className="font-bold text-[15px] leading-tight tracking-tight text-white">EduTrack</p>
            <span className={cn("text-[10px] font-semibold px-1.5 py-0.5 rounded-md inline-block mt-0.5 uppercase tracking-wide", rolePill[role])}>
              {roleLabel[role]}
            </span>
          </div>
        </div>
      ...
      {/* User footer */}
      <div className="p-3 flex-shrink-0" style={{ borderTop: "1px solid hsl(var(--sidebar-border))" }}>
        <div className="flex items-center gap-2.5 mb-2 px-1">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-semibold truncate text-white">{userName}</p>
            <p className="text-[11px] truncate" style={{ color: "hsl(var(--sidebar-text))" }}>{userEmail}</p>
          </div>
        </div>
        <button
          ...
          onMouseLeave={e => { e.currentTarget.style.background = ""; e.currentTarget.style.color = "hsl(var(--sidebar-text))"; }}
        >
...
export function Sidebar(props: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Mobile overlay */}
      {mobileOpen && <div className="sidebar-overlay" onClick={close} aria-hidden />}

      {/* Mobile drawer */}
      <aside
        className={cn("sidebar-drawer fixed inset-y-0 left-0 z-50 w-[220px] flex flex-col shadow-2xl lg:hidden sidebar-root", mobileOpen ? "translate-x-0" : "-translate-x-full")}
      >
